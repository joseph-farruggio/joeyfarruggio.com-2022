...
(self => {
    ...
    const taskQueue = [];
    let asyncMsgTimer = 0;
    const queue = (instance, $applyPath$, callType, $assignInstanceId$, $groupedGetters$, buffer, task) => {
        task = {
        ...
        task.$debug$ = ((target, applyPath, callType) => {
        ...
        if (3 === callType) {
            ...
            taskQueue.push(task);
            buffer && console.error("buffer must be sent NonBlockingNoSideEffect");
            if (1 === callType) {
    ...
    const addStorageApi = (win, storageName, items) => {
        let getIndexByKey = key => items.findIndex((i => i[STORAGE_KEY] === key));
        let index;
        let item;
        let storage = {
            getItem(key) {
    ...
    class WorkerEventTargetProxy extends WorkerProxy {}
    ...
    const lazyLoadMedia = () => {
        if (!self.ptm) {
            self.ptm = [ getter, setter, callMethod, constructGlobal, definePrototypePropertyDescriptor, randomId, WorkerProxy, WorkerEventTargetProxy, WinIdKey, InstanceIdKey, ApplyPathKey ];
    ...
    const run = (env, scriptContent, scriptUrl) => new Function(`with(this){${scriptContent.replace(/\bthis\b/g, "thi$(this)").replace(/\/\/# so/g, "//Xso")};function thi$(t){return t===this?window:t}}` + (scriptUrl ? "\n//# sourceURL=" + scriptUrl : "")).call(env.$window$);
    ...
    class CSSStyleDeclaration extends WorkerProxy {
    ...
    const defineWorkerInterface = ([cstrName, superCstrName, members, interfaceType, nodeName]) => {
        const SuperCstr = TrapConstructors[cstrName] ? WorkerTrapProxy : "EventTarget" === superCstrName ? WorkerEventTargetProxy : "Object" === superCstrName ? WorkerProxy : self[superCstrName];
